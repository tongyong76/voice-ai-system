...
**下一步开发建议**:
1. 在 GPU 服务器上端到端测试推理链路 (需要安装 PyTorch + FunASR 模型)
2. 完善任务调度 — 任务下发后通过 WebSocket 通知设备开始采集
3. 前端 `npm install` 安装依赖后进行 UI 联调
4. 实现说话人批量导入的前端界面
5. 补充 `AudioList` 的分页 total 和 `AudioDetail` 的波形播放联调
